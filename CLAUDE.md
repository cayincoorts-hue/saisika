...
## ⛔ 铁律：任何重大新想法必须先调研（不可跳过）

**用户提出新功能、新方向、或较大改动时，第一步不是写代码，而是：**

1. `WebSearch` 搜索：有没有人做过类似的东西？怎么做的？
2. 找到至少 1-2 个参考案例或竞品
3. 总结：别人做对了什么、做错了什么、我们能借鉴什么
4. 把调研结果展示给用户，等确认后再动手

**典型场景：**
- 用户说"能不能加个 XX 功能" → 先搜 XX 功能的现有方案
- 用户说"搞个 YY 设计" → 先搜 YY 风格的设计参考
- 用户说"用 ZZ 技术" → 先搜 ZZ 的最佳实践和坑
